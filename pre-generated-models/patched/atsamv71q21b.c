...
#define TYPE_ATSAMV71Q21B_UART "atsamv71q21b-uart"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BUART, ATSAMV71Q21B_UART)

struct ATSAMV71Q21BUART {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_UART_SIZE				0xe8
...
static void atsamv71q21b_uart_update(ATSAMV71Q21BUART *t) {
	int conditions = (((t->IMR & 0x1) && (t->SR & 0x1)) || ((t->IMR & 0x2) && (t->SR & 0x2)) || ((t->SR & 0x200) && (t->IMR & 0x200)));
	qemu_set_irq(t->irq[0], conditions);

}

static int atsamv71q21b_uart_can_receive(void *opaque) {
	return 1;
}

static void atsamv71q21b_uart_receive(void *opaque, const uint8_t *buf, int size) {
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(opaque);

	t->RHR = *buf;
	t->SR |= 0x1;

	atsamv71q21b_uart_update(t);
}

static gboolean atsamv71q21b_uart_transmit(void *do_not_use, GIOCondition cond, void *opaque) {
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(opaque);
	int ret;
	...
	if (ret <= 0){
		t->watch_tag = qemu_chr_fe_add_watch(&(t->chr), G_IO_OUT | G_IO_HUP, atsamv71q21b_uart_transmit, t);
		if (!t->watch_tag) {
			goto buffer_drained;
		}
...
static void atsamv71q21b_uart_register_reset(ATSAMV71Q21BUART *t) {
	t->SR |= 0x2;

}

static uint64_t atsamv71q21b_uart_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B UART read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_uart_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(opaque);
	...
	switch (offset) {
		...
		case A_UART_THR:
			t->THR = value;
			atsamv71q21b_uart_transmit(NULL, G_IO_OUT, t);
			break;
		case A_UART_BRGR:
			t->BRGR = value;
			break;
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B UART write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_uart_ops = {
	.read = atsamv71q21b_uart_read,
	.write = atsamv71q21b_uart_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_uart_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_uart_ops, t, "atsamv71q21b-uart", ATSAMV71Q21B_UART_SIZE);
...
static void atsamv71q21b_uart_realize(DeviceState *dev, Error **errp) {
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(dev);

	qemu_chr_fe_set_handlers(
		&(t->chr), atsamv71q21b_uart_can_receive, atsamv71q21b_uart_receive,
		NULL, NULL, t, NULL, true);

}

static void atsamv71q21b_uart_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BUART *t = ATSAMV71Q21B_UART(obj);
	atsamv71q21b_uart_register_reset(t);
}

static const VMStateDescription atsamv71q21b_uart_vmstate = {
	.name = "atsamv71q21b-uart",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(CR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(MR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(IER, ATSAMV71Q21BUART),
		VMSTATE_UINT32(IDR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(IMR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(SR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(RHR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(THR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(BRGR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(CMPR, ATSAMV71Q21BUART),
		VMSTATE_UINT32(WPMR, ATSAMV71Q21BUART),

		VMSTATE_END_OF_LIST()
	}
};

static Property atsamv71q21b_uart_properties[] = {
	DEFINE_PROP_CHR("chardev", ATSAMV71Q21BUART, chr),
	DEFINE_PROP_END_OF_LIST()
};

static void atsamv71q21b_uart_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_uart_vmstate;
	dc->realize = atsamv71q21b_uart_realize;
	rc->phases.enter = atsamv71q21b_uart_reset_enter;
	device_class_set_props(dc, atsamv71q21b_uart_properties);

}

static const TypeInfo atsamv71q21b_uart_info = {
	.name = TYPE_ATSAMV71Q21B_UART,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BUART),
	.instance_init = atsamv71q21b_uart_init,
	.class_init = atsamv71q21b_uart_class_init,
};

static void atsamv71q21b_uart_register_types(void) {
	type_register_static(&atsamv71q21b_uart_info);
}

type_init(atsamv71q21b_uart_register_types);

#define TYPE_ATSAMV71Q21B_USART "atsamv71q21b-usart"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BUSART, ATSAMV71Q21B_USART)

struct ATSAMV71Q21BUSART {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_USART_SIZE				0xec
...
__attribute__((unused))
static void atsamv71q21b_usart_update(ATSAMV71Q21BUSART *t) {
...
static int atsamv71q21b_usart_can_receive(void *opaque) {
	return 1;
}

static void atsamv71q21b_usart_receive(void *opaque, const uint8_t *buf, int size) {
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(opaque);

	t->US_RHR = *buf;
	t->US_CSR_USART_MODE |= 0x1;

	atsamv71q21b_usart_update(t);
}

static gboolean atsamv71q21b_usart_transmit(void *do_not_use, GIOCondition cond, void *opaque) {
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(opaque);
	int ret;
	...
	if (ret <= 0){
		t->watch_tag = qemu_chr_fe_add_watch(&(t->chr), G_IO_OUT | G_IO_HUP, atsamv71q21b_usart_transmit, t);
		if (!t->watch_tag) {
			goto buffer_drained;
		}
...
static void atsamv71q21b_usart_register_reset(ATSAMV71Q21BUSART *t) {
	t->US_CSR_USART_MODE |= 0x2;

}

static uint64_t atsamv71q21b_usart_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B USART read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_usart_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(opaque);
	...
	switch (offset) {
		...
		case A_USART_US_IER_USART_MODE:
			...
			// PERRY PATCH
			t->US_IMR_USART_MODE |= value; atsamv71q21b_usart_update(t);
			break;
		...
		case A_USART_US_THR:
			t->US_THR = value;
			atsamv71q21b_usart_transmit(NULL, G_IO_OUT, t);
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B USART write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_usart_ops = {
	.read = atsamv71q21b_usart_read,
	.write = atsamv71q21b_usart_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_usart_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_usart_ops, t, "atsamv71q21b-usart", ATSAMV71Q21B_USART_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_usart_realize(DeviceState *dev, Error **errp) {
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(dev);

	qemu_chr_fe_set_handlers(
		&(t->chr), atsamv71q21b_usart_can_receive, atsamv71q21b_usart_receive,
		NULL, NULL, t, NULL, true);

}

static void atsamv71q21b_usart_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BUSART *t = ATSAMV71Q21B_USART(obj);
	atsamv71q21b_usart_register_reset(t);
}

static const VMStateDescription atsamv71q21b_usart_vmstate = {
	.name = "atsamv71q21b-usart",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(US_CR_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_CR_SPI_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_CR_LIN_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_MR_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_MR_SPI_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IER_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IER_SPI_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IER_LIN_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IER_LON_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IDR_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IDR_SPI_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IDR_LIN_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IDR_LON_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IMR_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IMR_SPI_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IMR_LIN_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IMR_LON_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_CSR_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_CSR_SPI_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_CSR_LIN_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_CSR_LON_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_RHR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_THR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_BRGR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_RTOR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_TTGR_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_TTGR_LON_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_FIDI_USART_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_FIDI_LON_MODE, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_NER, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IF, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_MAN, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LINMR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LINIR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LINBRR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONMR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONPR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONDL, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONL2HDR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONBL, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONB1TX, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONB1RX, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_LONPRIO, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IDTTX, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_IDTRX, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_ICDIFF, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_WPMR, ATSAMV71Q21BUSART),
		VMSTATE_UINT32(US_WPSR, ATSAMV71Q21BUSART),

		VMSTATE_END_OF_LIST()
	}
};

static Property atsamv71q21b_usart_properties[] = {
	DEFINE_PROP_CHR("chardev", ATSAMV71Q21BUSART, chr),
	DEFINE_PROP_END_OF_LIST()
};

static void atsamv71q21b_usart_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_usart_vmstate;
	dc->realize = atsamv71q21b_usart_realize;
	rc->phases.enter = atsamv71q21b_usart_reset_enter;
	device_class_set_props(dc, atsamv71q21b_usart_properties);

}

static const TypeInfo atsamv71q21b_usart_info = {
	.name = TYPE_ATSAMV71Q21B_USART,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BUSART),
	.instance_init = atsamv71q21b_usart_init,
	.class_init = atsamv71q21b_usart_class_init,
};

static void atsamv71q21b_usart_register_types(void) {
	type_register_static(&atsamv71q21b_usart_info);
}

type_init(atsamv71q21b_usart_register_types);

#define TYPE_ATSAMV71Q21B_PMC "atsamv71q21b-pmc"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BPMC, ATSAMV71Q21B_PMC)

struct ATSAMV71Q21BPMC {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_PMC_SIZE				0x148
...
REG32(PMC_SLPWK_ER1, 0x134)
	...
	FIELD(PMC_SLPWK_ER1, PID53, 21, 1)
...
REG32(PMC_SLPWK_DR1, 0x138)
	...
	FIELD(PMC_SLPWK_DR1, PID53, 21, 1)
...
REG32(PMC_SLPWK_SR1, 0x13c)
	...
	FIELD(PMC_SLPWK_SR1, PID53, 21, 1)
...
REG32(PMC_SLPWK_ASR1, 0x140)
	...
	FIELD(PMC_SLPWK_ASR1, PID53, 21, 1)
...
static void atsamv71q21b_pmc_register_reset(ATSAMV71Q21BPMC *t) {

}

static uint64_t atsamv71q21b_pmc_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BPMC *t = ATSAMV71Q21B_PMC(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B PMC read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_pmc_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BPMC *t = ATSAMV71Q21B_PMC(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B PMC write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_pmc_ops = {
	.read = atsamv71q21b_pmc_read,
	.write = atsamv71q21b_pmc_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_pmc_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BPMC *t = ATSAMV71Q21B_PMC(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_pmc_ops, t, "atsamv71q21b-pmc", ATSAMV71Q21B_PMC_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_pmc_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_pmc_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BPMC *t = ATSAMV71Q21B_PMC(obj);
	atsamv71q21b_pmc_register_reset(t);
}

static const VMStateDescription atsamv71q21b_pmc_vmstate = {
	.name = "atsamv71q21b-pmc",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(SCER, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SCDR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SCSR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCER0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCDR0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCSR0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(CKGR_UCKR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(CKGR_MOR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(CKGR_MCFR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(CKGR_PLLAR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(MCKR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(USB, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(IER, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(IDR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(IMR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(FSMR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(FSPR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(FOCR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(WPMR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(WPSR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCER1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCDR1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCSR1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(OCR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_ER0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_DR0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_SR0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_ASR0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PMMR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_ER1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_DR1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_SR1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_ASR1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(SLPWK_AIPR, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK0, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK1, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK2, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK3, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK4, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK5, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK6, ATSAMV71Q21BPMC),
		VMSTATE_UINT32(PCK7, ATSAMV71Q21BPMC),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_pmc_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_pmc_vmstate;
	dc->realize = atsamv71q21b_pmc_realize;
	rc->phases.enter = atsamv71q21b_pmc_reset_enter;

}

static const TypeInfo atsamv71q21b_pmc_info = {
	.name = TYPE_ATSAMV71Q21B_PMC,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BPMC),
	.instance_init = atsamv71q21b_pmc_init,
	.class_init = atsamv71q21b_pmc_class_init,
};

static void atsamv71q21b_pmc_register_types(void) {
	type_register_static(&atsamv71q21b_pmc_info);
}

type_init(atsamv71q21b_pmc_register_types);

#define TYPE_ATSAMV71Q21B_SUPC "atsamv71q21b-supc"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BSUPC, ATSAMV71Q21B_SUPC)

struct ATSAMV71Q21BSUPC {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_SUPC_SIZE				0x18
...
static void atsamv71q21b_supc_register_reset(ATSAMV71Q21BSUPC *t) {

}

static uint64_t atsamv71q21b_supc_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BSUPC *t = ATSAMV71Q21B_SUPC(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B SUPC read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_supc_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BSUPC *t = ATSAMV71Q21B_SUPC(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B SUPC write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_supc_ops = {
	.read = atsamv71q21b_supc_read,
	.write = atsamv71q21b_supc_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_supc_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BSUPC *t = ATSAMV71Q21B_SUPC(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_supc_ops, t, "atsamv71q21b-supc", ATSAMV71Q21B_SUPC_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_supc_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_supc_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BSUPC *t = ATSAMV71Q21B_SUPC(obj);
	atsamv71q21b_supc_register_reset(t);
}

static const VMStateDescription atsamv71q21b_supc_vmstate = {
	.name = "atsamv71q21b-supc",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(CR, ATSAMV71Q21BSUPC),
		VMSTATE_UINT32(SMMR, ATSAMV71Q21BSUPC),
		VMSTATE_UINT32(MR, ATSAMV71Q21BSUPC),
		VMSTATE_UINT32(WUMR, ATSAMV71Q21BSUPC),
		VMSTATE_UINT32(WUIR, ATSAMV71Q21BSUPC),
		VMSTATE_UINT32(SR, ATSAMV71Q21BSUPC),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_supc_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_supc_vmstate;
	dc->realize = atsamv71q21b_supc_realize;
	rc->phases.enter = atsamv71q21b_supc_reset_enter;

}

static const TypeInfo atsamv71q21b_supc_info = {
	.name = TYPE_ATSAMV71Q21B_SUPC,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BSUPC),
	.instance_init = atsamv71q21b_supc_init,
	.class_init = atsamv71q21b_supc_class_init,
};

static void atsamv71q21b_supc_register_types(void) {
	type_register_static(&atsamv71q21b_supc_info);
}

type_init(atsamv71q21b_supc_register_types);

#define TYPE_ATSAMV71Q21B_PIO "atsamv71q21b-pio"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BPIO, ATSAMV71Q21B_PIO)

struct ATSAMV71Q21BPIO {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_PIO_SIZE				0x168
...
__attribute__((unused))
static void atsamv71q21b_pio_update(ATSAMV71Q21BPIO *t) {
...
static int atsamv71q21b_pio_can_receive(void *opaque) {
	return 1;
}

static void atsamv71q21b_pio_receive(void *opaque, const uint8_t *buf, int size) {
	ATSAMV71Q21BPIO *t = ATSAMV71Q21B_PIO(opaque);

	t->IMR = (1 << (*buf % 32));
	t->ISR = (1 << (*buf % 32));

	atsamv71q21b_pio_update(t);
}

static void atsamv71q21b_pio_register_reset(ATSAMV71Q21BPIO *t) {

}

static uint64_t atsamv71q21b_pio_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BPIO *t = ATSAMV71Q21B_PIO(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B PIO read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_pio_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BPIO *t = ATSAMV71Q21B_PIO(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B PIO write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_pio_ops = {
	.read = atsamv71q21b_pio_read,
	.write = atsamv71q21b_pio_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_pio_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BPIO *t = ATSAMV71Q21B_PIO(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_pio_ops, t, "atsamv71q21b-pio", ATSAMV71Q21B_PIO_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_pio_realize(DeviceState *dev, Error **errp) {
	ATSAMV71Q21BPIO *t = ATSAMV71Q21B_PIO(dev);

	qemu_chr_fe_set_handlers(
		&(t->chr), atsamv71q21b_pio_can_receive, atsamv71q21b_pio_receive,
		NULL, NULL, t, NULL, true);

}

static void atsamv71q21b_pio_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BPIO *t = ATSAMV71Q21B_PIO(obj);
	atsamv71q21b_pio_register_reset(t);
}

static const VMStateDescription atsamv71q21b_pio_vmstate = {
	.name = "atsamv71q21b-pio",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(PER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(OER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ODR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(OSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IFER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IFDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IFSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(SODR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(CODR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ODSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PDSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IMR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ISR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(MDER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(MDDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(MDSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PUDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PUER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PUSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IFSCDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IFSCER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(IFSCSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(SCDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PPDDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PPDER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PPDSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(OWER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(OWDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(OWSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(AIMER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(AIMDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(AIMMR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ESR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(LSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ELSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(FELLSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(REHLSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(FRLHSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(LOCKSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(WPMR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(WPSR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(SCHMITT, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(DRIVER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PCMR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PCIER, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PCIDR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PCIMR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PCISR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(PCRHR, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ABCDSR0, ATSAMV71Q21BPIO),
		VMSTATE_UINT32(ABCDSR1, ATSAMV71Q21BPIO),

		VMSTATE_END_OF_LIST()
	}
};

static Property atsamv71q21b_pio_properties[] = {
	DEFINE_PROP_CHR("chardev", ATSAMV71Q21BPIO, chr),
	DEFINE_PROP_END_OF_LIST()
};

static void atsamv71q21b_pio_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_pio_vmstate;
	dc->realize = atsamv71q21b_pio_realize;
	rc->phases.enter = atsamv71q21b_pio_reset_enter;
	device_class_set_props(dc, atsamv71q21b_pio_properties);

}

static const TypeInfo atsamv71q21b_pio_info = {
	.name = TYPE_ATSAMV71Q21B_PIO,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BPIO),
	.instance_init = atsamv71q21b_pio_init,
	.class_init = atsamv71q21b_pio_class_init,
};

static void atsamv71q21b_pio_register_types(void) {
	type_register_static(&atsamv71q21b_pio_info);
}

type_init(atsamv71q21b_pio_register_types);

#define TYPE_ATSAMV71Q21B_EFC "atsamv71q21b-efc"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BEFC, ATSAMV71Q21B_EFC)

struct ATSAMV71Q21BEFC {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_EFC_SIZE				0xe8
...
static void atsamv71q21b_efc_register_reset(ATSAMV71Q21BEFC *t) {

}

static uint64_t atsamv71q21b_efc_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BEFC *t = ATSAMV71Q21B_EFC(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B EFC read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_efc_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BEFC *t = ATSAMV71Q21B_EFC(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B EFC write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_efc_ops = {
	.read = atsamv71q21b_efc_read,
	.write = atsamv71q21b_efc_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_efc_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BEFC *t = ATSAMV71Q21B_EFC(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_efc_ops, t, "atsamv71q21b-efc", ATSAMV71Q21B_EFC_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_efc_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_efc_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BEFC *t = ATSAMV71Q21B_EFC(obj);
	atsamv71q21b_efc_register_reset(t);
}

static const VMStateDescription atsamv71q21b_efc_vmstate = {
	.name = "atsamv71q21b-efc",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(EEFC_FMR, ATSAMV71Q21BEFC),
		VMSTATE_UINT32(EEFC_FCR, ATSAMV71Q21BEFC),
		VMSTATE_UINT32(EEFC_FSR, ATSAMV71Q21BEFC),
		VMSTATE_UINT32(EEFC_FRR, ATSAMV71Q21BEFC),
		VMSTATE_UINT32(EEFC_WPMR, ATSAMV71Q21BEFC),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_efc_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_efc_vmstate;
	dc->realize = atsamv71q21b_efc_realize;
	rc->phases.enter = atsamv71q21b_efc_reset_enter;

}

static const TypeInfo atsamv71q21b_efc_info = {
	.name = TYPE_ATSAMV71Q21B_EFC,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BEFC),
	.instance_init = atsamv71q21b_efc_init,
	.class_init = atsamv71q21b_efc_class_init,
};

static void atsamv71q21b_efc_register_types(void) {
	type_register_static(&atsamv71q21b_efc_info);
}

type_init(atsamv71q21b_efc_register_types);

#define TYPE_ATSAMV71Q21B_WDT "atsamv71q21b-wdt"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BWDT, ATSAMV71Q21B_WDT)

struct ATSAMV71Q21BWDT {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_WDT_SIZE				0xc
...
static void atsamv71q21b_wdt_register_reset(ATSAMV71Q21BWDT *t) {

}

static uint64_t atsamv71q21b_wdt_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BWDT *t = ATSAMV71Q21B_WDT(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B WDT read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_wdt_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BWDT *t = ATSAMV71Q21B_WDT(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B WDT write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_wdt_ops = {
	.read = atsamv71q21b_wdt_read,
	.write = atsamv71q21b_wdt_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_wdt_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BWDT *t = ATSAMV71Q21B_WDT(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_wdt_ops, t, "atsamv71q21b-wdt", ATSAMV71Q21B_WDT_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_wdt_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_wdt_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BWDT *t = ATSAMV71Q21B_WDT(obj);
	atsamv71q21b_wdt_register_reset(t);
}

static const VMStateDescription atsamv71q21b_wdt_vmstate = {
	.name = "atsamv71q21b-wdt",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(CR, ATSAMV71Q21BWDT),
		VMSTATE_UINT32(MR, ATSAMV71Q21BWDT),
		VMSTATE_UINT32(SR, ATSAMV71Q21BWDT),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_wdt_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_wdt_vmstate;
	dc->realize = atsamv71q21b_wdt_realize;
	rc->phases.enter = atsamv71q21b_wdt_reset_enter;

}

static const TypeInfo atsamv71q21b_wdt_info = {
	.name = TYPE_ATSAMV71Q21B_WDT,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BWDT),
	.instance_init = atsamv71q21b_wdt_init,
	.class_init = atsamv71q21b_wdt_class_init,
};

static void atsamv71q21b_wdt_register_types(void) {
	type_register_static(&atsamv71q21b_wdt_info);
}

type_init(atsamv71q21b_wdt_register_types);

#define TYPE_ATSAMV71Q21B_SMC "atsamv71q21b-smc"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BSMC, ATSAMV71Q21B_SMC)

struct ATSAMV71Q21BSMC {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_SMC_SIZE				0xec
...
static void atsamv71q21b_smc_register_reset(ATSAMV71Q21BSMC *t) {

}

static uint64_t atsamv71q21b_smc_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BSMC *t = ATSAMV71Q21B_SMC(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B SMC read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_smc_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BSMC *t = ATSAMV71Q21B_SMC(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B SMC write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_smc_ops = {
	.read = atsamv71q21b_smc_read,
	.write = atsamv71q21b_smc_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_smc_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BSMC *t = ATSAMV71Q21B_SMC(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_smc_ops, t, "atsamv71q21b-smc", ATSAMV71Q21B_SMC_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));

}

static void atsamv71q21b_smc_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_smc_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BSMC *t = ATSAMV71Q21B_SMC(obj);
	atsamv71q21b_smc_register_reset(t);
}

static const VMStateDescription atsamv71q21b_smc_vmstate = {
	.name = "atsamv71q21b-smc",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(OCMS, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(KEY1, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(KEY2, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(WPMR, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(WPSR, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER0_SETUP, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER0_PULSE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER0_CYCLE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER0_MODE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER1_SETUP, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER1_PULSE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER1_CYCLE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER1_MODE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER2_SETUP, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER2_PULSE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER2_CYCLE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER2_MODE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER3_SETUP, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER3_PULSE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER3_CYCLE, ATSAMV71Q21BSMC),
		VMSTATE_UINT32(SMC_CS_NUMBER3_MODE, ATSAMV71Q21BSMC),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_smc_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_smc_vmstate;
	dc->realize = atsamv71q21b_smc_realize;
	rc->phases.enter = atsamv71q21b_smc_reset_enter;

}

static const TypeInfo atsamv71q21b_smc_info = {
	.name = TYPE_ATSAMV71Q21B_SMC,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BSMC),
	.instance_init = atsamv71q21b_smc_init,
	.class_init = atsamv71q21b_smc_class_init,
};

static void atsamv71q21b_smc_register_types(void) {
	type_register_static(&atsamv71q21b_smc_info);
}

type_init(atsamv71q21b_smc_register_types);

#define TYPE_ATSAMV71Q21B_CHIPID "atsamv71q21b-chipid"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BCHIPID, ATSAMV71Q21B_CHIPID)

struct ATSAMV71Q21BCHIPID {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_CHIPID_SIZE				0x8
...
static void atsamv71q21b_chipid_register_reset(ATSAMV71Q21BCHIPID *t) {

}

static uint64_t atsamv71q21b_chipid_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BCHIPID *t = ATSAMV71Q21B_CHIPID(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B CHIPID read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_chipid_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BCHIPID *t = ATSAMV71Q21B_CHIPID(opaque);
	(void)t;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B CHIPID write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_chipid_ops = {
	.read = atsamv71q21b_chipid_read,
	.write = atsamv71q21b_chipid_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_chipid_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BCHIPID *t = ATSAMV71Q21B_CHIPID(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_chipid_ops, t, "atsamv71q21b-chipid", ATSAMV71Q21B_CHIPID_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));

}

static void atsamv71q21b_chipid_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_chipid_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BCHIPID *t = ATSAMV71Q21B_CHIPID(obj);
	atsamv71q21b_chipid_register_reset(t);
}

static const VMStateDescription atsamv71q21b_chipid_vmstate = {
	.name = "atsamv71q21b-chipid",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(CIDR, ATSAMV71Q21BCHIPID),
		VMSTATE_UINT32(EXID, ATSAMV71Q21BCHIPID),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_chipid_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_chipid_vmstate;
	dc->realize = atsamv71q21b_chipid_realize;
	rc->phases.enter = atsamv71q21b_chipid_reset_enter;

}

static const TypeInfo atsamv71q21b_chipid_info = {
	.name = TYPE_ATSAMV71Q21B_CHIPID,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BCHIPID),
	.instance_init = atsamv71q21b_chipid_init,
	.class_init = atsamv71q21b_chipid_class_init,
};

static void atsamv71q21b_chipid_register_types(void) {
	type_register_static(&atsamv71q21b_chipid_info);
}

type_init(atsamv71q21b_chipid_register_types);

#define TYPE_ATSAMV71Q21B_MATRIX "atsamv71q21b-matrix"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BMATRIX, ATSAMV71Q21B_MATRIX)

struct ATSAMV71Q21BMATRIX {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_MATRIX_SIZE				0x1ec
...
REG32(MATRIX_MATRIX_PR0_PRAS, 0x80)
	...
	FIELD(MATRIX_MATRIX_PR0_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR1_PRAS, 0x88)
	...
	FIELD(MATRIX_MATRIX_PR1_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR2_PRAS, 0x90)
	...
	FIELD(MATRIX_MATRIX_PR2_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR3_PRAS, 0x98)
	...
	FIELD(MATRIX_MATRIX_PR3_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR4_PRAS, 0xa0)
	...
	FIELD(MATRIX_MATRIX_PR4_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR5_PRAS, 0xa8)
	...
	FIELD(MATRIX_MATRIX_PR5_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR6_PRAS, 0xb0)
	...
	FIELD(MATRIX_MATRIX_PR6_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR7_PRAS, 0xb8)
	...
	FIELD(MATRIX_MATRIX_PR7_PRAS, M7PR, 28, 2)
...
REG32(MATRIX_MATRIX_PR8_PRAS, 0xc0)
	...
	FIELD(MATRIX_MATRIX_PR8_PRAS, M7PR, 28, 2)
...
static void atsamv71q21b_matrix_register_reset(ATSAMV71Q21BMATRIX *t) {

}

static uint64_t atsamv71q21b_matrix_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BMATRIX *t = ATSAMV71Q21B_MATRIX(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B MATRIX read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_matrix_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BMATRIX *t = ATSAMV71Q21B_MATRIX(opaque);
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B MATRIX write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_matrix_ops = {
	.read = atsamv71q21b_matrix_read,
	.write = atsamv71q21b_matrix_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_matrix_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BMATRIX *t = ATSAMV71Q21B_MATRIX(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_matrix_ops, t, "atsamv71q21b-matrix", ATSAMV71Q21B_MATRIX_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));

}

static void atsamv71q21b_matrix_realize(DeviceState *dev, Error **errp) {
	return;

}

static void atsamv71q21b_matrix_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BMATRIX *t = ATSAMV71Q21B_MATRIX(obj);
	atsamv71q21b_matrix_register_reset(t);
}

static const VMStateDescription atsamv71q21b_matrix_vmstate = {
	.name = "atsamv71q21b-matrix",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(MRCR, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(CCFG_CAN0, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(CCFG_SYSIO, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(CCFG_PCCR, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(CCFG_DYNCKG, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(CCFG_SMCNFCS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(WPMR, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(WPSR, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG0, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG1, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG2, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG3, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG4, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG5, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG6, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG7, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG8, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG9, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG10, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG11, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MCFG12, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG0, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG1, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG2, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG3, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG4, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG5, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG6, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG7, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(SCFG8, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR0_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR0_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR1_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR1_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR2_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR2_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR3_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR3_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR4_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR4_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR5_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR5_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR6_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR6_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR7_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR7_PRBS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR8_PRAS, ATSAMV71Q21BMATRIX),
		VMSTATE_UINT32(MATRIX_PR8_PRBS, ATSAMV71Q21BMATRIX),

		VMSTATE_END_OF_LIST()
	}
};

static void atsamv71q21b_matrix_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_matrix_vmstate;
	dc->realize = atsamv71q21b_matrix_realize;
	rc->phases.enter = atsamv71q21b_matrix_reset_enter;

}

static const TypeInfo atsamv71q21b_matrix_info = {
	.name = TYPE_ATSAMV71Q21B_MATRIX,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BMATRIX),
	.instance_init = atsamv71q21b_matrix_init,
	.class_init = atsamv71q21b_matrix_class_init,
};

static void atsamv71q21b_matrix_register_types(void) {
	type_register_static(&atsamv71q21b_matrix_info);
}

type_init(atsamv71q21b_matrix_register_types);

#define TYPE_ATSAMV71Q21B_DACC "atsamv71q21b-dacc"
OBJECT_DECLARE_SIMPLE_TYPE(ATSAMV71Q21BDACC, ATSAMV71Q21B_DACC)

struct ATSAMV71Q21BDACC {
	/* parent */
	SysBusDevice parent;
...
#define ATSAMV71Q21B_DACC_SIZE				0xec
...
static int atsamv71q21b_dacc_can_receive(void *opaque) {
	return 1;
}

static void atsamv71q21b_dacc_receive(void *opaque, const uint8_t *buf, int size) {
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(opaque);
...
static gboolean atsamv71q21b_dacc_transmit(void *do_not_use, GIOCondition cond, void *opaque) {
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(opaque);
	int ret;
	...
	if (ret <= 0){
		t->watch_tag = qemu_chr_fe_add_watch(&(t->chr), G_IO_OUT | G_IO_HUP, atsamv71q21b_dacc_transmit, t);
		if (!t->watch_tag) {
			goto buffer_drained;
		}
...
static void atsamv71q21b_dacc_register_reset(ATSAMV71Q21BDACC *t) {

}

static uint64_t atsamv71q21b_dacc_read(void *opaque, hwaddr offset, unsigned size) {
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(opaque);
	uint64_t ret;
	...
	switch (offset) {
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B DACC read: bad offset %x\n", (int)offset);
			ret = 0;
			break;
	}
	return ret;
...
static void atsamv71q21b_dacc_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(opaque);
	...
	switch (offset) {
		...
		case A_DACC_CDR0:
			t->CDR0 = value;
			atsamv71q21b_dacc_transmit(NULL, G_IO_OUT, t);
			break;
		case A_DACC_CDR1:
			t->CDR1 = value;
			atsamv71q21b_dacc_transmit(NULL, G_IO_OUT, t);
		...
		default:
			qemu_log_mask(LOG_GUEST_ERROR, "ATSAMV71Q21B DACC write: bad offset %x\n", (int)offset);
			break;
	}
}

static const MemoryRegionOps atsamv71q21b_dacc_ops = {
	.read = atsamv71q21b_dacc_read,
	.write = atsamv71q21b_dacc_write,
	.endianness = DEVICE_LITTLE_ENDIAN
};

static void atsamv71q21b_dacc_init(Object *obj) {
	SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(obj);
	memory_region_init_io(&(t->iomem), obj, &atsamv71q21b_dacc_ops, t, "atsamv71q21b-dacc", ATSAMV71Q21B_DACC_SIZE);
	sysbus_init_mmio(sbd, &(t->iomem));
	for (int i = 0; i < 1; ++i) {
		sysbus_init_irq(sbd, &(t->irq[i]));
	}
...
static void atsamv71q21b_dacc_realize(DeviceState *dev, Error **errp) {
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(dev);

	qemu_chr_fe_set_handlers(
		&(t->chr), atsamv71q21b_dacc_can_receive, atsamv71q21b_dacc_receive,
		NULL, NULL, t, NULL, true);

}

static void atsamv71q21b_dacc_reset_enter(Object *obj, ResetType type) {
	ATSAMV71Q21BDACC *t = ATSAMV71Q21B_DACC(obj);
	atsamv71q21b_dacc_register_reset(t);
}

static const VMStateDescription atsamv71q21b_dacc_vmstate = {
	.name = "atsamv71q21b-dacc",
	.version_id = 0,
	.minimum_version_id = 0,
	.fields = (VMStateField[]) {
		VMSTATE_UINT32(CR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(MR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(TRIGR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(CHER, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(CHDR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(CHSR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(IER, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(IDR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(IMR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(ISR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(ACR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(WPMR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(WPSR, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(CDR0, ATSAMV71Q21BDACC),
		VMSTATE_UINT32(CDR1, ATSAMV71Q21BDACC),

		VMSTATE_END_OF_LIST()
	}
};

static Property atsamv71q21b_dacc_properties[] = {
	DEFINE_PROP_CHR("chardev", ATSAMV71Q21BDACC, chr),
	DEFINE_PROP_END_OF_LIST()
};

static void atsamv71q21b_dacc_class_init(ObjectClass *oc, void *data) {
	DeviceClass *dc = DEVICE_CLASS(oc);
	ResettableClass *rc = RESETTABLE_CLASS(oc);
	dc->vmsd = &atsamv71q21b_dacc_vmstate;
	dc->realize = atsamv71q21b_dacc_realize;
	rc->phases.enter = atsamv71q21b_dacc_reset_enter;
	device_class_set_props(dc, atsamv71q21b_dacc_properties);

}

static const TypeInfo atsamv71q21b_dacc_info = {
	.name = TYPE_ATSAMV71Q21B_DACC,
	.parent = TYPE_SYS_BUS_DEVICE,
	.instance_size = sizeof(ATSAMV71Q21BDACC),
	.instance_init = atsamv71q21b_dacc_init,
	.class_init = atsamv71q21b_dacc_class_init,
};

static void atsamv71q21b_dacc_register_types(void) {
	type_register_static(&atsamv71q21b_dacc_info);
}

type_init(atsamv71q21b_dacc_register_types);

#define TYPE_ATSAMV71Q21B_MACHINE MACHINE_TYPE_NAME("atsamv71q21b")
OBJECT_DECLARE_TYPE(ATSAMV71Q21BMachineState, ATSAMV71Q21BMachineClass, ATSAMV71Q21B_MACHINE)

struct ATSAMV71Q21BMachineClass {
	MachineClass parent;
};

struct ATSAMV71Q21BMachineState {
	MachineState parent;
	ARMv7MState armv7m;
};

static void atsamv71q21b_periph_init(MachineState *machine) {
	ATSAMV71Q21BMachineState *sms = ATSAMV71Q21B_MACHINE(machine);
	ATSAMV71Q21BUSART *p0 = g_new(ATSAMV71Q21BUSART, 1);
	object_initialize_child(OBJECT(sms), "USART0", p0, TYPE_ATSAMV71Q21B_USART);
	...
	ATSAMV71Q21BUSART *p1 = g_new(ATSAMV71Q21BUSART, 1);
	object_initialize_child(OBJECT(sms), "USART1", p1, TYPE_ATSAMV71Q21B_USART);
	...
	ATSAMV71Q21BUSART *p2 = g_new(ATSAMV71Q21BUSART, 1);
	object_initialize_child(OBJECT(sms), "USART2", p2, TYPE_ATSAMV71Q21B_USART);
	...
	ATSAMV71Q21BUART *p3 = g_new(ATSAMV71Q21BUART, 1);
	object_initialize_child(OBJECT(sms), "UART0", p3, TYPE_ATSAMV71Q21B_UART);
	...
	ATSAMV71Q21BUART *p4 = g_new(ATSAMV71Q21BUART, 1);
	object_initialize_child(OBJECT(sms), "UART1", p4, TYPE_ATSAMV71Q21B_UART);
	...
	ATSAMV71Q21BUART *p5 = g_new(ATSAMV71Q21BUART, 1);
	object_initialize_child(OBJECT(sms), "UART2", p5, TYPE_ATSAMV71Q21B_UART);
	...
	ATSAMV71Q21BUART *p6 = g_new(ATSAMV71Q21BUART, 1);
	object_initialize_child(OBJECT(sms), "UART3", p6, TYPE_ATSAMV71Q21B_UART);
	...
	ATSAMV71Q21BUART *p7 = g_new(ATSAMV71Q21BUART, 1);
	object_initialize_child(OBJECT(sms), "UART4", p7, TYPE_ATSAMV71Q21B_UART);
	...
	ATSAMV71Q21BPIO *p8 = g_new(ATSAMV71Q21BPIO, 1);
	object_initialize_child(OBJECT(sms), "PIOA", p8, TYPE_ATSAMV71Q21B_PIO);
	...
	ATSAMV71Q21BPIO *p9 = g_new(ATSAMV71Q21BPIO, 1);
	object_initialize_child(OBJECT(sms), "PIOB", p9, TYPE_ATSAMV71Q21B_PIO);
	...
	ATSAMV71Q21BPIO *p10 = g_new(ATSAMV71Q21BPIO, 1);
	object_initialize_child(OBJECT(sms), "PIOC", p10, TYPE_ATSAMV71Q21B_PIO);
	...
	ATSAMV71Q21BPIO *p11 = g_new(ATSAMV71Q21BPIO, 1);
	object_initialize_child(OBJECT(sms), "PIOD", p11, TYPE_ATSAMV71Q21B_PIO);
	...
	ATSAMV71Q21BPIO *p12 = g_new(ATSAMV71Q21BPIO, 1);
	object_initialize_child(OBJECT(sms), "PIOE", p12, TYPE_ATSAMV71Q21B_PIO);
	...
	ATSAMV71Q21BPMC *p13 = g_new(ATSAMV71Q21BPMC, 1);
	object_initialize_child(OBJECT(sms), "PMC", p13, TYPE_ATSAMV71Q21B_PMC);
	...
	ATSAMV71Q21BSUPC *p14 = g_new(ATSAMV71Q21BSUPC, 1);
	object_initialize_child(OBJECT(sms), "SUPC", p14, TYPE_ATSAMV71Q21B_SUPC);
	...
	ATSAMV71Q21BEFC *p15 = g_new(ATSAMV71Q21BEFC, 1);
	object_initialize_child(OBJECT(sms), "EFC", p15, TYPE_ATSAMV71Q21B_EFC);
	...
	ATSAMV71Q21BWDT *p16 = g_new(ATSAMV71Q21BWDT, 1);
	object_initialize_child(OBJECT(sms), "WDT", p16, TYPE_ATSAMV71Q21B_WDT);
	...
	ATSAMV71Q21BSMC *p17 = g_new(ATSAMV71Q21BSMC, 1);
	object_initialize_child(OBJECT(sms), "SMC", p17, TYPE_ATSAMV71Q21B_SMC);
	sysbus_realize(SYS_BUS_DEVICE(p17), &error_fatal);
	p17->base = 0x40080000;
	sysbus_mmio_map(SYS_BUS_DEVICE(p17), 0, 0x40080000);

	ATSAMV71Q21BCHIPID *p18 = g_new(ATSAMV71Q21BCHIPID, 1);
	object_initialize_child(OBJECT(sms), "CHIPID", p18, TYPE_ATSAMV71Q21B_CHIPID);
	sysbus_realize(SYS_BUS_DEVICE(p18), &error_fatal);
	p18->base = 0x400e0940;
	sysbus_mmio_map(SYS_BUS_DEVICE(p18), 0, 0x400e0940);

	ATSAMV71Q21BMATRIX *p19 = g_new(ATSAMV71Q21BMATRIX, 1);
	object_initialize_child(OBJECT(sms), "MATRIX", p19, TYPE_ATSAMV71Q21B_MATRIX);
	sysbus_realize(SYS_BUS_DEVICE(p19), &error_fatal);
	p19->base = 0x40088000;
	sysbus_mmio_map(SYS_BUS_DEVICE(p19), 0, 0x40088000);

	ATSAMV71Q21BDACC *p20 = g_new(ATSAMV71Q21BDACC, 1);
	object_initialize_child(OBJECT(sms), "DACC", p20, TYPE_ATSAMV71Q21B_DACC);
...
static void atsamv71q21b_common_init(MachineState *machine) {
	ATSAMV71Q21BMachineState *sms = ATSAMV71Q21B_MACHINE(machine);
	...
	atsamv71q21b_periph_init(machine);

	armv7m_load_kernel(ARM_CPU(first_cpu), machine->kernel_filename, 0x400000, 0x200000);
}

static void atsamv71q21b_class_init(ObjectClass *oc, void *data) {
	MachineClass *mc = MACHINE_CLASS(oc);
	mc->desc = "atsamv71q21b";
	mc->init = atsamv71q21b_common_init;
	mc->max_cpus = 1;
	mc->default_cpus = 1;
	mc->default_cpu_type = ARM_CPU_TYPE_NAME("cortex-m7");
}

static const TypeInfo atsamv71q21b_info = {
	.name = TYPE_ATSAMV71Q21B_MACHINE,
	.parent = TYPE_MACHINE,
	.instance_size = sizeof(ATSAMV71Q21BMachineState),
	.class_size = sizeof(ATSAMV71Q21BMachineClass),
	.class_init = atsamv71q21b_class_init
};

static void atsamv71q21b_machine_init(void) {
	type_register_static(&atsamv71q21b_info);
}

type_init(atsamv71q21b_machine_init);
